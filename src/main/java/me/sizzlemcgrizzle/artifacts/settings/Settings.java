...
public class Settings {
    ...
    private static File SETTINGS_FILE = new File(ArtifactsPlugin.getInstance().getDataFolder(), "settings.yml");
    
    public static void load() {
        if (!SETTINGS_FILE.exists())
            ArtifactsPlugin.getInstance().saveResource(SETTINGS_FILE.getName(), true);
        
        YamlConfiguration config = YamlConfiguration.loadConfiguration(SETTINGS_FILE);
        
        PREFIX = ChatColor.translateAlternateColorCodes('&', config.getString("Prefix", "&8[&5Artifacts&8]&e "));
